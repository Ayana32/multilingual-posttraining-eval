...
def test_pilot_sample_is_reproducible_across_calls():
    loader = PolyGuardPromptsLoader()
    first = [i.parallel_item_id for i in loader.load("en", limit=PILOT_LIMIT, seed=PILOT_SEED)]
    second = [i.parallel_item_id for i in loader.load("en", limit=PILOT_LIMIT, seed=PILOT_SEED)]
    assert first == second


def test_pilot_sample_contains_both_classes():
    loader = PolyGuardPromptsLoader()
    ...
    assert sum(counts.values()) == PILOT_LIMIT
...
def test_pilot_sample_is_a_valid_subset_of_the_full_english_benchmark():
    loader = PolyGuardPromptsLoader()
    full_ids = {i.parallel_item_id for i in loader.load("en")}
    pilot_ids = {i.parallel_item_id for i in loader.load("en", limit=PILOT_LIMIT, seed=PILOT_SEED)}
    assert pilot_ids <= full_ids
    assert len(pilot_ids) == PILOT_LIMIT
...
def test_full_english_load_marks_exactly_the_documented_null_label_rows_as_none():
    ...
    could silently drift from the actual data."""
    loader = PolyGuardPromptsLoader()
